...
def encode_sentence_to_ids(sentence, token_to_id, unk_token='<unk>'):
    ...
    for word in words:
        if word in token_to_id:
            result.append(token_to_id[word])
        else:
            result.append(token_to_id[unk_token])
    return result
...
def stack_padded_sequences_to_batch(padded_sequences):
    ...
    seq_tensor = torch.Tensor(padded_sequences).to(torch.long)
    return seq_tensor
...
def compute_positional_div_term(d_model):
    ...
    result = torch.ones(d_model // 2) * (10000.0**(-2.0 * torch.arange(d_model//2) / d_model))
    return result
...
def build_position_index_column(max_len):
    ...
    return torch.arange(max_len).to(torch.float32).view(-1, 1)
...
def build_sinusoidal_positional_encoding(max_len, d_model):
    ...
    positions = torch.arange(max_len).view(-1, 1)

    w = torch.exp(-torch.arange(0, d_model, 2) * math.log(10000) / d_model)

    pe[:, ::2] = torch.sin(positions * w)
...
def build_causal_mask(seq_len):
    ...
    causal_mask = torch.where(causal_mask == 1, True, False)
...
def mask_attention_scores_with_neg_inf(scores, mask):
    ...
    return torch.where(mask == True, scores, float('-inf'))
...
def softmax_attention_weights(masked_scores):
    ...
    res = softmax(masked_scores, dim=-1)
    return torch.where(torch.isnan(res), 0.0, res)
...
def transpose_heads_before_sequence(split_tensor):
    ...
    return split_tensor.transpose(2, 1)
...
def merge_heads_and_project_output(context, w_o, b_o):
    ...
    res = merge_heads_back_to_model_dim(context) @ w_o.T
    if b_o is not None:
        res += b_o
    return res
...
def apply_ffn_first_linear_and_relu(x, w1, b1):
    ...
    B, L, _ = x.shape
    _, d_ff = w1.shape
    zeros = torch.zeros(B, L, d_ff)
    return torch.max(x @ w1 + b1, zeros)
...
def compute_layer_norm_mean_and_variance(x):
    ...
    return torch.mean(x, dim=-1, keepdim=True), torch.var(x, dim=-1, correction=0, keepdim=True)
...
def apply_dropout_with_keep_mask(x, keep_mask, keep_prob):
    ...
    return x * torch.where(keep_mask == True, 1.0, 0.0) * (1 / keep_prob)
...
def encoder_layer_self_attention_sublayer(x, w_q, w_k, w_v, w_o, gamma, beta, num_heads, src_mask):
    ...
    output = assemble_multi_head_attention_forward(x, x, x, w_q, w_k, w_v, w_o, num_heads, src_mask)
...
def assemble_encoder_layer(x, layer_params, num_heads, src_mask):
    ...
    h = encoder_layer_self_attention_sublayer(x, w_q, w_k, w_v, w_o, attn_gamma, attn_beta, num_heads, src_mask)

    return encoder_layer_feed_forward_sublayer(h, w1, b1, w2, b2, ffn_gamma, ffn_beta)
...
def decoder_layer_cross_attention_sublayer(y, encoder_output, w_q, w_k, w_v, w_o, gamma, beta, num_heads, src_mask):
    ...
    output = assemble_multi_head_attention_forward(y, encoder_output, encoder_output, w_q, w_k, w_v, w_o, num_heads, src_mask)
    
    return apply_residual_add_and_norm(y, output, gamma, beta)
...
def assemble_decoder_layer(y, encoder_output, layer_params, num_heads, src_mask, tgt_mask):
    ...
    encoder_output = decoder_layer_masked_self_attention_sublayer(
        y, w_q_self, w_k_self, w_v_self, w_o_self,
        self_gamma, self_beta, num_heads, tgt_mask
    )
    h = decoder_layer_cross_attention_sublayer(
        y, encoder_output, w_q_cross, w_k_cross, w_v_cross, w_o_cross,
        cross_gamma, cross_beta, num_heads, src_mask
    )
    
    return decoder_layer_feed_forward_sublayer(y, w1, b1, w2, b2, ffn_gamma, ffn_beta)
...
def apply_log_softmax_over_vocab(logits):
    ...
    return logits - torch.log(torch.sum(torch.exp(logits), dim=-1, keepdim=True))
...
def run_transformer_forward(src_ids, tgt_ids, model_params, num_heads, pad_id):
    ...
    src_seq = src_ids.shape[1]
    tgt_seq = tgt_ids.shape[1]

    seq_len = max(src_seq, tgt_seq)
